...
Robustness upgrades (CI / hostile-reviewer hardening):
  ...
  2) Deterministic run_hash = sha256(canonical JSON of *raw YAML(qpt) dict* after normalisation.
     (This MUST match qpt_fit.py exactly.)
  3) Require qpt/output/run_manifest.json written each run (records run_hash + params_path + raw qpt params).
...
# -----------------------------------------------------------------------------#
# Hash / manifest utilities (MUST match qpt_fit.py)
# -----------------------------------------------------------------------------#
def _to_builtin(x: Any) -> Any:
    """
    Convert common YAML / numpy containers into JSON-stable Python builtins.
    Must match the conversion used in qpt_fit.py.
    """
    ...
    # JSON can handle: str, int, float, bool, None
    return x


def _normalize_params(qpt_params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Normalise params to a canonical JSON-hashable dict:
      - ensure keys are strings
      - recursively convert to builtins
    """
    if not isinstance(qpt_params, dict):
        raise ValueError("qpt params must be a dict for hashing.")
    return _to_builtin(qpt_params)
...
def _compute_run_hash(qpt_params_raw: Dict[str, Any]) -> str:
    """
    Compute sha256 over canonical JSON of the *raw YAML(qpt)* dict after normalisation.
    This MUST match qpt_fit.py, which hashes the same object.
    """
    norm = _normalize_params(qpt_params_raw)
    return hashlib.sha256(_canonical_json_bytes(norm)).hexdigest()
...
def load_params(config_path: Optional[str] = None) -> Tuple[Dict[str, Any], Dict[str, Any], str]:
    """
    Load YAML and return:
      - raw_qpt: exactly the dict under YAML key 'qpt' (or whole file if no key)
      - effective: DEFAULTS merged with raw_qpt and with derived fields resolved for computation
      - cfg_path: resolved YAML path (provenance)
    """
    cfg_path = _resolve_config_path(config_path)
    with open(cfg_path, "r", encoding="utf-8-sig", errors="replace") as f:
        cfg = yaml.safe_load(f) or {}
    raw_qpt = (cfg.get("qpt") or cfg or {})
    if not isinstance(raw_qpt, dict):
        raise ValueError("QPT YAML did not parse to a dict under key 'qpt'.")

    # Effective params used for simulation
    p: Dict[str, Any] = {**DEFAULTS, **raw_qpt}
    ...
    # Coerce numerics / types (effective only; raw_qpt remains unchanged for hashing)
    ...
    return raw_qpt, p, cfg_path


def simulate_populations(
    t_s: np.ndarray,
    gamma_fwd: float,
    gamma_b_vals: list[float],
    kappa0: float
) -> np.ndarray:
    """
...
def main() -> None:
    ...
    raw_qpt, p, cfg_path = load_params()
    ...
    # IMPORTANT: run_hash is computed from *raw YAML(qpt) dict*, not the effective merged dict.
    run_hash = _compute_run_hash(raw_qpt)
    ...
    manifest = {
        "run_hash": run_hash,
        "params_path": os.path.abspath(cfg_path),
        "timestamp_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        # Store the *normalised* raw params to make inspection deterministic across platforms
        "qpt": _normalize_params(raw_qpt),
    }
    ...
    np.savez(
        ...
        params_path=np.array(os.path.abspath(cfg_path)),
        t=t.astype(float),
