...
Robustness upgrades (CI / hostile-reviewer hardening):
  ...
  B) Require qpt/output/run_manifest.json and enforce:
       - YAML(qpt) -> run_hash matches manifest.run_hash (or manifest.yaml_hash)
       - qpt_sim_data.npz run_hash (if present) matches manifest hash
...
# -----------------------------------------------------------------------------#
# Hash / manifest utilities (MUST match qpt_simulation.py semantics)
# -----------------------------------------------------------------------------#
def _to_builtin(obj: Any) -> Any:
    """
    Convert numpy / pandas-ish types to JSON-serialisable Python builtins,
...
def _normalize_params(qpt_params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Deterministic normalisation used *only* for hashing.

    Key decisions:
      - Drop keys with value None (common source of hash drift across scripts).
      - Convert numpy types to Python builtins.
      - Keep strings as-is; keep dict keys as strings.
    """
    p = _to_builtin(qpt_params)
    if not isinstance(p, dict):
        raise ValueError("qpt params must be a dict for hashing.")

    # Drop None-valued keys to avoid 'None vs missing' drift
...
def _canonical_json_bytes(obj: Any) -> bytes:
    """
    Canonical JSON representation: stable key ordering and separators.
    """
...
def _compute_run_hash(qpt_params: Dict[str, Any]) -> str:
    """
    Hash the *normalised* YAML(qpt) dict. This must match the simulator's
    definition of run_hash for CI to pass.
    """
    norm = _normalize_params(qpt_params)
...
def _enforce_manifest_and_hashes(
    ...
    """
    Enforce:
      - run_manifest.json exists and contains a hash (run_hash or yaml_hash)
    ...
    # Accept either key, but prefer run_hash if present.
    if "run_hash" in manifest:
        run_hash = str(manifest["run_hash"])
    elif "yaml_hash" in manifest:
        run_hash = str(manifest["yaml_hash"])
    else:
        raise RuntimeError("run_manifest.json missing key 'run_hash' (or 'yaml_hash').")

    yaml_hash = _compute_run_hash(params_yaml_qpt)
...
def main() -> None:
    ...
    # Keep absolute x = λ_env (s^-1); slope has units of seconds.
