...
def _read_aic_lrt():
    ...
    return {
        "metric": metric, "m1": m1, "m2": m2, "delta": delta,
        "lrt": lrt, "pval": pval, "df": df, "lrt_valid": lrt_valid, "n": n_eff,
    }
...
def main():
    ...
    # 3-line summary textbox
    lrt_line = (f"LRT={lrt:.2f} (χ²₍{df}₎), p={p:.3g}" if np.isfinite(lrt) and np.isfinite(p)
                else "LRT: n/a")
    extra = f"  n={n_eff}" if np.isfinite(n_eff) else ""
    wilks_line = "Wilks valid." if lrt_valid else "Wilks approx."
    boundary_line = "mixture boundary \u2192 p approx."  # → arrow
    ...
    axB.set_title("Log-survival curvature (early, ms)", fontsize=11, pad=1.5)
    ...
    lo = [ci.loc["lo", col_eta], ci.loc["lo", col_tf] * 1e3, ci.loc["lo", col_ts] * 1e3]
    hi = [ci.loc["hi", col_eta], ci.loc["hi", col_tf] * 1e3, ci.loc["hi", col_ts] * 1e3]
    ...
    fig.suptitle("Tier-B tempered mixtures: 1-exp vs 2-exp", fontsize=12)
    # reserve a minimal gap between suptitle and axes
    fig.set_constrained_layout_pads(h_pad=0.02, w_pad=0.02, hspace=0.02, wspace=0.28)
    ...
    x_lab = max(0.002, bboxA.x0 - 0.062)     # a hair to the left of axA
    y_lab = min(0.998, bboxA.y1 + 0.030)     # a hair above axA
    fig.text(
        x_lab, y_lab, "(c)",
    ...
    for ext in ("png", "pdf"):
        fig.savefig(
            OUT_FIG / f"tierB_tempered_modelcomp.{ext}",
