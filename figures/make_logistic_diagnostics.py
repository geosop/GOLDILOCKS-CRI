...
- Bottom legend moved INSIDE (lower-right) with smaller fontsize.
- Metrics text split into two lines, top-left, tight line spacing.
- Removed extra right margin (since legend is now inside).
"""
import os, yaml
import numpy as np
import pandas as pd
...
def main():
    ...
    fig.subplots_adjust(right=0.98)  # near-full width; keep a tiny right margin
    ...
    # ---- Metrics text: two lines, top-left, tight spacing ----
    ...
    # ---- Legend INSIDE (lower-right) and one size smaller ----
    leg2 = ax2.legend(
        loc='lower right',
        frameon=True, fancybox=True,
        fontsize=4,                 # one step smaller than rcParams (5)
        handlelength=1.3, labelspacing=0.25, borderpad=0.25
    )
    frame2 = leg2.get_frame()
    frame2.set_alpha(0.95)
    frame2.set_edgecolor('0.80')
    leg2.set_zorder(5)
