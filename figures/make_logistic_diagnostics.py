...
Tweak: ALL axes now show labels **with units**.
- Top: q (–), G(q) (–)   [dimensionless]
- Bottom: Predicted probability (–), Observed rate (–)   [dimensionless]
"""
import os, yaml
import numpy as np
import pandas as pd
...
mpl.rcParams.update({
    ...
    "axes.linewidth": 0.6,
    "lines.linewidth": 1.0,
    "legend.fontsize": 5,     # global default; ax2 overrides to 4 below
    "xtick.labelsize": 7,
    "ytick.labelsize": 7,
    "pdf.fonttype": 42,
    "ps.fonttype": 42,
...
def main():
    ...
    # Figure: legend is inside, so use full width (no reserved right margin)
    ...
    ax1.set_xlabel(r"$q$ (–)")            # <-- units added
    ax1.set_ylabel(r"$G(q)$ (–)")         # <-- units added
    ...
    def _clean_calib(df):
        ...
        return df[(df["err_lo"].notna()) & (df["err_hi"].notna())]
    ...
    # Metrics text (unchanged)
    def metric_val(name, a):
        row = metr[(metr['metric']==name) & (metr['a']==a)]
        return float(row['value'].iloc[0]) if not row.empty else np.nan

    if two:
        line1 = f"a1: Brier={metric_val('Brier','a1'):.3f}, ECE={metric_val('ECE','a1'):.3f}"
        line2 = f"a2: Brier={metric_val('Brier','a2'):.3f}, ECE={metric_val('ECE','a2'):.3f}"
        text  = line1 + "\n" + line2
    else:
    ...
    ax2.set_xlabel("Predicted probability (–)")  # <-- units added
    ax2.set_ylabel("Observed rate (–)")          # <-- units added

    # Legend INSIDE (lower-right), one size smaller
    leg2 = ax2.legend(
        loc='lower right',
    ...
    fig.savefig(png, dpi=int(params.get('figure_dpi', 1200)),     # high-dpi raster
                bbox_inches='tight')
    plt.close(fig)
