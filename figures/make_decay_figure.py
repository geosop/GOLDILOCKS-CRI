...
def _parse_args():
    ...
    # Provenance rendering controls (layout defaults updated)
    ...
    ap.add_argument("--prov-loc", default=os.getenv("CRI_PROV_LOC", "bottom-left"),
    ...
    # NEW: explicit figure-coordinate placement for provenance (for “as-left-as-possible”)
    ap.add_argument("--prov-x", type=float, default=float(os.getenv("CRI_PROV_X", 0.005)),
    ...
    # NEW: shift x-axis label right to avoid collisions
    ap.add_argument("--xlabel-x", type=float, default=float(os.getenv("CRI_XLABEL_X", 0.99)),
...
def main():
    ...
    fig.subplots_adjust(top=0.965, bottom=0.19)
    ...
    # Detection bound
    ax.axhline(
        lnA_min, linestyle="--", linewidth=1.0, zorder=0,
    ...
    # Visible provenance footer (NOW left-anchored and explicitly placeable)
    if bool(args.show_provenance):
        ...
        # default anchor derived from loc, but allow explicit prov-x/prov-y for “as-left-as-possible”
        x = float(args.prov_x) if args.prov_x is not None else (0.01 if "left" in loc else 0.99)
        y = float(args.prov_y) if args.prov_y is not None else (0.012 if "bottom" in loc else 0.98)
