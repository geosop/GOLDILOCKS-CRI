# -*- coding: utf-8 -*-
"""
figures/make_decay_figure.py  •  CRI v0.3-SIM (robust)

Generates Box-2(a) decay figure from decay/output artifacts produced by:
  - decay/simulate_decay.py   (decay_data.csv + run_manifest.json)
  - decay/fit_decay.py        (decay_band.csv + fit_decay_results.csv)

Robustness upgrades / hostile-reviewer hardening:
  1) Deterministic path resolution (repo root inferred from this file).
  2) Config override via env var CRI_DECAY_CONFIG (same policy as decay scripts).
  3) Enforce presence of decay/output/run_manifest.json and require that
     YAML-derived *simulation-effective* decay params hash matches manifest.run_hash.
  4) Embed run_hash + params_path into PDF metadata and PNG metadata.
  5) Avoid hard-coded colors (Matplotlib defaults) for CI-safety and style neutrality.
  6) Fail fast with actionable errors if expected inputs are missing or malformed.

Outputs:
  - figures/output/Box2a_decay.pdf
  - figures/output/Box2a_decay.png
(or alternative stem via --out-stem / CRI_BOX2A_STEM)

Notes:
  - x-axis uses milliseconds (ms) but fitting is done in seconds (s).
  - y-axis is log amplitude: ln A_pre(τ_f), dimensionless (–).
"""
...
def _enforce_manifest_and_get_run_hash(params_path: str, sim_effective: Dict[str, Any]) -> Tuple[str, str]:
    """
    Require decay/output/run_manifest.json and enforce YAML hash match.
    Returns (run_hash, manifest_params_path).
    """
    ...
    # If manifest carries params_path, enforce that it matches the resolved path
    ...
    return run_hash, man_params_path
...
def _load_tau_fut_seconds(decay_output_dir: str) -> Tuple[float, float, float]:
    """
    Source of truth: decay/output/fit_decay_results.csv produced by decay/fit_decay.py.
    """
...
def _resolve_detection_threshold(p_raw: Dict[str, Any], band_df: pd.DataFrame) -> Tuple[float, float]:
    """
    Returns (A_min, lnA_min).
    detection_mode:
      - 'param' (default): use YAML A_min / epsilon_detection
      - 'auto': set lnA_min to the lower band at the maximum delay (most conservative at longest τ_f)
    """
    mode = str(p_raw.get("detection_mode", "param")).lower().strip()
    A_min = p_raw.get("A_min", p_raw.get("epsilon_detection", 0.01))
    A_min = _safe_float(A_min, 0.01)

    if mode == "auto":
        # choose the last x in the band
...
# -----------------------------------------------------------------------------
def _parse_args():
    ap = argparse.ArgumentParser(description="Box-2(a) decay figure (RSOS-ready, manifest-verified).")
    ...
    ap.add_argument("--ann-x", type=float, default=float(os.getenv("CRI_ANN_X", 0.66)))
    ap.add_argument("--ann-yfrac", type=float, default=float(os.getenv("CRI_ANN_YFRAC", 0.55)))
...
def main():
    ...
    run_hash, man_params_path = _enforce_manifest_and_get_run_hash(params_path, sim_effective)

    # --- Inputs from decay/output ---
    pts = _require_csv(os.path.join(DECAY_OUT, "decay_data.csv"), required_cols={"delta", "lnA_pre"})
    if "se_lnA" not in pts.columns:
        # permissible, but we prefer it for RSOS and WLS
        pass

    band = _require_csv(os.path.join(DECAY_OUT, "decay_band.csv"),
    ...
    if {"lnA_pre", "se_lnA"}.issubset(pts.columns):
        ...
        ax.scatter(pts["delta_ms"], pts["lnA_pre"], s=16, zorder=3, label="Sampled delays")

    # Detection bound in log-domain
    ax.axhline(
        lnA_min,
        linestyle="--",
        linewidth=1.0,
    ...
    # Axes labels (units explicit)
    ax.set_xlabel(r"$\tau_f$ (ms)")
    ax.set_ylabel(r"$\ln A_{\mathrm{pre}}(\tau_f)$ (–)")

    # x-limits from YAML (pad for aesthetics)
    ...
    ci_line = rf"\n$[{lo_s*1e3:.1f},{hi_s*1e3:.1f}]\,\mathrm{{ms}}$" if (np.isfinite(lo_s) and np.isfinite(hi_s)) else ""
    ann_text = (
    ...
    # Inset (raw amplitude) with units
    ...
    ax_ins.scatter(pts["delta_ms"], A_pts, s=12, zorder=3)
    ax_ins.axhline(A_min, linestyle="--", linewidth=0.7, zorder=1)
    ...
    # PNG metadata supported via pil_kwargs in newer Matplotlib; harmless if ignored
    meta_png = {
        "Software": "CRI pipeline",
        "Description": f"run_hash={run_hash}; params_path={os.path.abspath(params_path)}",
    }
    ...
    fig.savefig(
        ...
        bbox_inches="tight",
        pad_inches=0.01,
        pil_kwargs={"pnginfo": None}  # avoid hard dependency on PIL.PngInfo creation
    )
