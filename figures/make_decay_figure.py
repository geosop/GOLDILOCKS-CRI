...
def _load_tau_fut_seconds(decay_output_dir):
    ...
    if os.path.exists(f1):
        try:
            df = pd.read_csv(f1)
            if 'tau_hat_ms' in df.columns: tau_s = float(df['tau_hat_ms'].iloc[0]) / 1000.0
            if 'ci_lo_ms'  in df.columns: lo_s  = float(df['ci_lo_ms'].iloc[0])  / 1000.0
            if 'ci_hi_ms'  in df.columns: hi_s  = float(df['ci_hi_ms'].iloc[0])  / 1000.0
        except Exception:
            pass
    if not np.isfinite(tau_s):
        ...
        if os.path.exists(f2):
            try:
                ...
                if {'name','center_s'}.issubset(band_meta.columns):
    ...
    for fname in ('decay_fit.csv', 'decay_summary.csv'):
        if not np.isfinite(tau_s):
            ...
            if os.path.exists(f):
                try:
                    df = pd.read_csv(f)
                    if 'tau_ms'   in df.columns: tau_s = float(df['tau_ms'].iloc[0])   / 1000.0
                    if 'ci_lo_ms' in df.columns: lo_s  = float(df['ci_lo_ms'].iloc[0]) / 1000.0
                    if 'ci_hi_ms' in df.columns: hi_s  = float(df['ci_hi_ms'].iloc[0]) / 1000.0
                except Exception:
                    pass
...
def parse_args():
    p = argparse.ArgumentParser(description="Decay figure with units on axes (RSOS-ready).")
    p.add_argument("--inset-x", type=float, default=float(os.getenv("CRI_INSET_X", 0.14)))
    p.add_argument("--inset-y", type=float, default=float(os.getenv("CRI_INSET_Y", 0.28)))
    p.add_argument("--inset-w", type=float, default=float(os.getenv("CRI_INSET_W", 0.22)))
...
def main():
    ...
    A_pts     = np.exp(pts['lnA_pre'].values)          # dimensionless
    A_central = np.exp(band['lnA_central'].values)     # dimensionless
