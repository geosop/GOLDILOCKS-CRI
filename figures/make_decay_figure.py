...
What this version FIXES (provenance):
  1) run_hash is enforced against decay/output/run_manifest.json AND YAML-effective params.
  2) run_hash + resolved config path are rendered VISIBLY on the figure (footer).
  3) run_hash + config path are embedded into:
       - PDF metadata (matplotlib native)
       - PNG metadata (Pillow text chunks, when Pillow available)
...
def _resolve_detection_threshold(p_raw: Dict[str, Any], band_df: pd.DataFrame) -> Tuple[float, float]:
    ...
    detection_mode:
      ...
      - 'auto': set lnA_min to the lower band at the maximum delay
    """
...
def main():
    ...
    run_hash, man_params_path, manifest = _enforce_manifest_and_get_run_hash(params_path, sim_effective)
    ...
    # Leave room for footer provenance inside the canvas (so bbox_inches="tight" won't cut it)
    ...
    if bool(args.show_provenance):
        ...
        # Keep the full absolute path in metadata; keep footer short & readable
        prov_text = f"run_hash={shown_hash}…  |  cfg={shown_cfg}"

        loc = str(args.prov_loc).lower()
    ...
    # For PNG: embed text chunks when Pillow available
    pnginfo = None
    if _HAVE_PIL:
        try:
