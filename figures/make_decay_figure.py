...
OCT-19 update:
- NEW: Position controls for the inset that do NOT change its size:
    --inset-x, --inset-y  (env: CRI_INSET_X, CRI_INSET_Y)
  (sizes still controlled only by --inset-w, --inset-h)
- NEW: Position controls for slope/τ̂_fut annotation:
    --ann-x (axes coords), --ann-yfrac (fraction of y-range)
    (env: CRI_ANN_X, CRI_ANN_YFRAC)
- Removed the previous hardcoded +0.05 x-offset applied to the inset.
- Added clamping to keep inset inside axes bounds.

Other behavior unchanged:
- DejaVu Sans; τ̂_fut read from decay/output/*; CRI reference line slope = -1/τ̂_fut
- Wide 95% CI band, orange samples, red detection bound; legend UR
- Inset defaults placed LL; X-axis 0–20 ms; save with bbox_inches='tight'
"""
import os
import yaml
import argparse
...
def parse_args():
    ...
    parser.add_argument("--inset-h", type=float, default=float(os.getenv("CRI_INSET_H", 0.20)),
...
def main():
    ...
    # Slope/τ annotation (positionable)
    ...
    # ---------------- Inset (adjustable position, fixed size unless w/h changed) ----
    inset_w_pct = f"{args.inset_w*100:.0f}%"
    inset_h_pct = f"{args.inset_h*100:.0f}%"

    # Clamp the lower-left corner so the inset stays inside the axes
