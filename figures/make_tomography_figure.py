...
def design_t_stat(lambda_max, n_levels, sigma_R, kappa0):
    """Back-of-envelope expected t for slope vs 0."""
...
def main():
    ...
    fig.text(
        0.01, 0.99, r'(\mathit{c})', transform=fig.transFigure,
    ...
    # Panel label "(c)" at top-left of LEFT panel only
    ax1.text(0.015, 0.985, "(c)", transform=ax1.transAxes,
             ha="left", va="top", fontsize=9, fontweight="bold", zorder=6)
    ...
    for f in [leg1.get_frame()]:
        f.set_facecolor(LIGHT_GRAY); f.set_edgecolor("0.80"); f.set_linewidth(0.6)
    ...
    # RIGHT: R vs λ with units
    order = np.argsort(x_right)
    xr = x_right[order]
    Rl = np.asarray(R_low)[order]; Rh = np.asarray(R_high)[order]; Rm = np.asarray(R_mean)[order]
    ...
    ax2.set_ylabel(r"Jump-weight ratio $R$ (–)")  # dimensionless
    ...
    if not args.no_annot:
        ...
        ax2.text(0.02, 0.98, stats_txt, transform=ax2.transAxes,
                 ha="left", va="top",
