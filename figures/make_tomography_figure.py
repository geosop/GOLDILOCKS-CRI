...
def main():
    ...
    # Legend: move to BOTTOM-RIGHT inside the right panel, with minimal inter-line spacing
    leg2 = ax2.legend(
        loc="lower right",
        frameon=True, fancybox=True,
        labelspacing=0.2,      # tighter vertical spacing between entries
        borderpad=0.25,        # tighter padding inside legend frame
        handlelength=1.2,
        handletextpad=0.35,
        columnspacing=0.6
    )
    ...
    if not args.no_annot:
        # Stats "inset": move from bottom-right → TOP-LEFT and tighten line spacing
        stats_txt = (
