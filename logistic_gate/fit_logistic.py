...
Reads:
  ...
  - YAML config used for Box2b (resolved with same precedence as simulate_logistic.py)
...
Writes:
  ...
  - logistic_gate/output/logistic_kernel.csv (optional)
...
Anti-contradiction mechanisms:
  - Enforces run_manifest.json presence and consistency:
      ...
      * manifest.run_hash matches YAML-derived run_hash (computed with SAME defaulting rules as simulate_logistic.py)
      * (optional) manifest.config_sha256 matches YAML file bytes
...
def _apply_simulate_defaults(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    This dict is what must be hashed to reproduce the manifest.run_hash.
    """
    p = dict(p)  # copy
...
def _apply_fit_defaults(p: Dict[str, Any]) -> Dict[str, Any]:
    """
    Fit-only defaults (bootstrap/diagnostics/initial guesses).
    IMPORTANT: these MUST NOT be included in the run_hash comparison logic,
    so we apply them only after hashing checks pass.
    """
    p = dict(p)  # copy
...
def parse_args():
    ap = argparse.ArgumentParser(description="Fit Box-2(b) logistic gate model to simulated trials.")
    ap.add_argument(
        "--config",
...
def kernel_smoother(q_grid, q_obs, y_obs, h, return_neff: bool = False):
    h = max(float(h), 1e-6)
    q_obs = np.asarray(q_obs, dtype=float)
    y_obs = np.asarray(y_obs, dtype=float)
    preds = np.empty_like(q_grid, dtype=float)
    neff  = np.empty_like(q_grid, dtype=float)
    for i, q0 in enumerate(q_grid):
        w = gaussian_kernel((q0 - q_obs) / h)
        sw = float(np.sum(w))
        preds[i] = float(np.sum(w * y_obs) / sw) if sw > 0 else np.nan
        sw2 = float(np.sum(w * w))
        neff[i] = (sw * sw / sw2) if sw2 > 0 else np.nan

    return (preds, neff) if return_neff else preds      
...
def _require_manifest_and_hashes(
    ...
    params_sim_effective: Dict[str, Any],
    params_path: str,
    atol_a: float = 1e-6,
) -> str:
    """
    ...
    # Internal manifest self-consistency (if logistic_params is present)
    if "logistic_params" in manifest and isinstance(manifest["logistic_params"], dict):
        ...
        if rh_internal != run_hash_manifest:
            raise RuntimeError(
    ...
    # Optional: verify YAML file bytes match manifest config_sha256 (strong drift check)
    if "config_sha256" in manifest and isinstance(manifest["config_sha256"], str):
        ...
        if sha_now != str(manifest["config_sha256"]):
            raise RuntimeError(
                ...
                "This indicates the config file changed after generating outputs.\n"
                "Fix: delete logistic_gate/output/* and rerun simulate_logistic.py with the intended YAML."
            )

    # Check YAML->hash matches manifest hash (using simulate-defaulted dict)
    run_hash_yaml = _compute_run_hash(params_sim_effective)
    if run_hash_yaml != run_hash_manifest:
        raise RuntimeError(
            "YAML-derived run_hash does not match run_manifest.json.\n"
            f"  YAML run_hash:     {run_hash_yaml}\n"
            f"  manifest run_hash: {run_hash_manifest}\n"
            "This indicates your outputs were generated from a different effective YAML than the one currently resolved.\n"
            "Fix: delete logistic_gate/output/* and rerun simulate_logistic.py using the intended YAML."
        )
    ...
    if "run_hash" in df_trials.columns:
        ...
        if len(uniq) != 1:
            raise RuntimeError(
                ...
                "This suggests mixed outputs. Delete logistic_gate/output/* and rerun pipeline."
            )
    ...
    # Check a-levels match YAML a1/a2 when use_two_conditions is on
    use_two = bool(params_sim_effective.get("use_two_conditions", True))
    ...
    if use_two:
        ...
        a1_y, a2_y = float(params_sim_effective["a1"]), float(params_sim_effective["a2"])
...
def _resolve_a1_a2(df_trials: pd.DataFrame, p_sim: dict) -> tuple[float, float]:
    a_vals = _extract_unique_arousal(df_trials)
    if len(a_vals) < 2:
        raise RuntimeError("Two-condition fit requested but only one arousal level present.")

    a1_y, a2_y = float(p_sim["a1"]), float(p_sim["a2"])
...
# Fitting pipelines
# -----------------------------------------------------------------------------#
def fit_two_condition(df_trials: pd.DataFrame, p_fit: dict, p_sim: dict):
    a1, a2 = _resolve_a1_a2(df_trials, p_sim)

    q = df_trials["q"].values.astype(float)
    y = df_trials["y"].values.astype(float)
    a = df_trials["a"].values.astype(float)

    theta0 = np.array([p_fit.get("p0_guess_a1", 0.5),
                       p_fit.get("p0_guess_a2", 0.52),
                       p_fit.get("alpha_guess", 0.05)], dtype=float)
    bounds = [(0.0, 1.0), (0.0, 1.0), (1e-6, None)]
    rng = np.random.default_rng(int(p_fit["seed"]))
    ...
    qs = np.linspace(float(p_sim["q_min"]), float(p_sim["q_max"]), int(p_sim["n_points"]))
    ...
    alpha_ci = (100.0 - float(p_fit["ci_percent"])) / 100.0
    ...
    kernel_csv = None
    if bool(p_fit.get("kernel_enabled", True)):
        h = float(p_fit.get("kernel_bandwidth", 0.08))
        n_b = int(p_fit.get("kernel_bootstrap", min(1000, n_boot)))
        grid = qs

        Gk1, neff1 = kernel_smoother(grid, q1, y1, h, return_neff=True)
        Gk2, neff2 = kernel_smoother(grid, q2, y2, h, return_neff=True)
        ...
        kernel_csv = pd.DataFrame({
            ...
            "n_eff_a1": neff1,
            "n_eff_a2": neff2,
            "kernel_bandwidth": float(h),        
        })

    # Calibration
    calib_bins = max(2, int(p_fit.get("calib_bins", 10)))
...
def fit_single_condition(df_trials: pd.DataFrame, p_fit: dict, p_sim: dict):
    q = df_trials["q"].values.astype(float)
    y = df_trials["y"].values.astype(float)

    theta0 = np.array([p_fit.get("p0_guess_a1", 0.5), p_fit.get("alpha_guess", 0.05)], dtype=float)
    ...
    if bool(p_fit.get("kernel_enabled", True)):
        h = float(p_fit.get("kernel_bandwidth", 0.08))
        n_b = int(p_fit.get("kernel_bootstrap", min(1000, n_boot)))
        Gk, neff = kernel_smoother(qs, q, y, h, return_neff=True)        
        B = []
        for _ in range(n_b):
            idx = rng.integers(len(q), size=len(q))
        ...
        kernel_csv = pd.DataFrame({
            "q": qs,
            "Gk_central_a1": Gk, "Gk_low_a1": lo_k, "Gk_high_a1": hi_k,
            "n_eff_a1": neff,
            "kernel_bandwidth": float(h),
        })
...
def main():
    ...
    # Resolve + load YAML (same precedence as simulate)
    params_path = _resolve_params_path(args.config)
    p_raw = _load_yaml_logistic_section(params_path)

    # Apply simulate defaults for hashing comparisons
    p_sim = _apply_simulate_defaults(p_raw)

    # Apply fit-only defaults for fitting/diagnostics
    p_fit = _apply_fit_defaults(p_sim)
    ...
    run_hash = _require_manifest_and_hashes(out_dir, df_trials, p_sim, params_path)

    a_levels = _extract_unique_arousal(df_trials)
    two = bool(p_sim.get("use_two_conditions", True)) and (len(a_levels) >= 2)

    if two:
        df_res, df_band, df_der, df_kernel, df_calib, df_metrics, a1_used, a2_used = fit_two_condition(df_trials, p_fit, p_sim)
        df_res["a1_used"] = float(a1_used)
        df_res["a2_used"] = float(a2_used)
    else:
        df_res, df_band, df_der, df_kernel, df_calib, df_metrics = fit_single_condition(df_trials, p_fit, p_sim)
        df_res["a1_used"] = float(a_levels[0]) if len(a_levels) else np.nan
