...
if [[ "${CRI_AUTO_INSTALL,,}" == "1" || "${CRI_AUTO_INSTALL,,}" == "true" || "${CRI_AUTO_INSTALL,,}" == "yes" ]]; then
  ...
  if [ -f "$REQ_IN" ]; then
    ...
    | grep -v -E '^(#|$)' \
    | grep -viE '^(pywin32|pywinpty|win32_setctime|win_inet_pton|PyQt5(-sip)?|PySide6|shiboken6)$' \
    | grep -viE 'opencv-python' \
    | grep -viE '^mne-qt-browser$' \
    > "$REQ_OUT"
...
if [[ -f decay/output/wls_tobit_coverage.csv ]]; then
  ...
  sed -n '1,80p' decay/output/wls_tobit_coverage.csv || true
fi
if [[ -f decay/output/wls_tobit_replicates.csv ]]; then
