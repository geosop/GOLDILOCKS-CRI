...
# -*- coding: utf-8 -*-
"""
decay/wls_tobit_robustness.py  •  CRI v0.3-SIM (robust; provenance-visible; MANIFEST-SOURCED HASH)
...
CRITICAL PROVENANCE GUARANTEE (this version):
  - The run_hash shown/stamped is taken ONLY from decay/output/run_manifest.json,
    i.e., the same source of truth used by figures/make_decay_figure.py.
  - YAML-derived hashes are used only for verification (must match manifest),
    never for display/stamping.
...
DECAY_OUT = os.path.join(HERE, "output")  # == decay/output
FIGD = os.path.join(os.path.dirname(HERE), "figures", "output")
os.makedirs(DECAY_OUT, exist_ok=True)
...
def _effective_sim_params_from_yaml(p_raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Note: CRI_SEED override is part of the *effective* sim dict and therefore affects the hash.
    """
...
# -----------------------------------------------------------------------------#
# Manifest: single source of truth for run_hash
# -----------------------------------------------------------------------------#
def _load_manifest_or_die(decay_out: str) -> dict:
    ...
    if not str(manifest.get("run_hash", "")).strip():
        raise RuntimeError("run_manifest.json missing key 'run_hash' (or empty).")
    return manifest


def _enforce_manifest_matches_yaml(manifest: dict, sim_effective: Dict[str, Any], params_path: str) -> None:
    """
    Verification only. Provenance display always uses manifest.run_hash.
    """
    ...
    if yaml_hash != run_hash:
        raise RuntimeError(
            ...
            "Fix: delete decay/output/* and re-run decay/simulate_decay.py with the intended YAML (and seed env)."
        )

    man_params_path = manifest.get("params_path", None)
    if man_params_path not in (None, "", "NA"):
        if os.path.abspath(str(man_params_path)) != os.path.abspath(params_path):
            raise RuntimeError(
...
# -----------------------------------------------------------------------------#
# Plotting with visible provenance (manifest run_hash)
# -----------------------------------------------------------------------------#
...
def main():
    ...
    ap.add_argument(
        "--data",
        default=os.path.join(DECAY_OUT, "decay_data.csv"),
        help="Aggregated dataset with columns: delta, lnA_pre, se_lnA (default: decay/output/decay_data.csv)",
    )
    ap.add_argument(
        "--allow-external-data",
        action="store_true",
        default=False,
        help="Allow --data to point outside decay/output/decay_data.csv (default: off, to guarantee same-run provenance).",
    )
    ap.add_argument(
        "--out-csv",
        default=os.path.join(DECAY_OUT, "wls_tobit_robustness.csv"),
    ...
    # Load manifest FIRST: single source of truth for provenance hash/path
    manifest = _load_manifest_or_die(DECAY_OUT)
    run_hash = str(manifest["run_hash"])
    manifest_params_path = str(manifest.get("params_path", "")) or ""

    # Load YAML (for verification and fit settings)
    p_raw, params_path = _load_yaml_decay_block(args.config)
    sim_effective = _effective_sim_params_from_yaml(p_raw)

    # Verify YAML-effective hash matches manifest hash (but never display YAML hash)
    _enforce_manifest_matches_yaml(manifest, sim_effective, params_path)

    # Strong guarantee: default data path must be the run's dataset unless explicitly allowed
    expected_data = os.path.join(DECAY_OUT, "decay_data.csv")
    data_path = os.path.abspath(args.data)
    if (not args.allow_external_data) and (os.path.abspath(expected_data) != data_path):
        raise RuntimeError(
            "To guarantee the same run_hash as Box-2(a), this script must fit the dataset from the same run.\n"
            f"Expected: {expected_data}\n"
            f"Got:      {args.data}\n"
            "Either omit --data, or pass --allow-external-data if you intentionally want a different dataset."
        )
    ...
    if not os.path.isfile(args.data):
        raise FileNotFoundError(f"Expected dataset not found: {args.data}\nRun decay/simulate_decay.py first.")
    ...
    # Stamp provenance into CSV using MANIFEST hash/path
    out_row = {
        "run_hash": run_hash,
        "params_path": os.path.abspath(manifest_params_path or params_path),
        "data_path": os.path.abspath(args.data),
        **out,
    ...
    _plot_tau_panel(
        ...
        args.out_pdf,
        run_hash=run_hash,
        params_path=(manifest_params_path or params_path),
    ...
    # Optional Monte-Carlo repeats (new datasets) keep the SAME run_hash provenance,
    # but are clearly separate artifacts; they do not change the primary figure/hash.
    R = int(args.repeats)
    if R > 0:
        rows = []
        for r in range(R):
        ...
        MC_path = os.path.join(DECAY_OUT, "wls_tobit_replicates.csv")
        ...
        SUM_path = os.path.join(DECAY_OUT, "wls_tobit_coverage.csv")
