...
Robust artifacts showing WLS/Tobit consistency with OLS CIs
on the SAME synthetic dataset produced by simulate_decay.py (Box-2a).

Changes: Figure now includes explicit axis labels and units:
...
def _plot_tau_panel(out: dict, out_png: str, out_pdf: str):
    ...
    # Categorical rows (top→bottom for readability)
    rows = [("OLS", 2), ("WLS", 1), ("Tobit", 0)]

    # OLS 95% CI drawn on its own row (solid), and as a light dashed guide on others
    ax.hlines(rows[0][1], out["ols_ci_lo_ms"], out["ols_ci_hi_ms"],
              color="C0", linewidth=4, label="OLS 95% sim. bootstrap CI", zorder=1)
    for _, y in rows[1:]:
        ax.hlines(y, out["ols_ci_lo_ms"], out["ols_ci_hi_ms"],
                  color="C0", linewidth=2, alpha=0.25, linestyle="--", zorder=0)

    # Point estimates
    ax.plot(out["tau_ols_ms"],   rows[0][1], marker="o", ms=7, color="C0",
            linestyle="None", label=f"OLS {out['tau_ols_ms']:.1f} ms", zorder=3)
    ax.plot(out["tau_wls_ms"],   rows[1][1], marker="^", ms=7, color="C1",
            linestyle="None", label=f"WLS {out['tau_wls_ms']:.1f} ms", zorder=3)
    ax.plot(out["tau_tobit_ms"], rows[2][1], marker="s", ms=7, color="C2",
            linestyle="None", label=f"Tobit {out['tau_tobit_ms']:.1f} ms", zorder=3)
    ...
    x_vals = [out["ols_ci_lo_ms"], out["ols_ci_hi_ms"],
              out["tau_ols_ms"], out["tau_wls_ms"], out["tau_tobit_ms"]]
    xmin, xmax = min(x_vals), max(x_vals)
    pad = 0.04 * (xmax - xmin if xmax > xmin else 1.0)
    ax.set_xlim(xmin - pad, xmax + pad)

    # Light x-grid to help reading the CI location
    ax.grid(axis="x", linestyle=":", alpha=0.5)
    ax.set_ylim(-0.6, 2.6)

    # One-row legend above the axes
    ...
    plt.tight_layout()
    fig.savefig(out_png, dpi=600, bbox_inches="tight")  # high-dpi raster
