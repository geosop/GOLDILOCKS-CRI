...
def _plot_tau_panel(out: dict, out_png: str):
    """Small visual: OLS 95% CI bar and markers for OLS/WLS/Tobit τ."""
    fig, ax = plt.subplots(figsize=(6.0, 2.5))
    y = 0.0
    ax.hlines(y, out["ols_ci_lo_ms"], out["ols_ci_hi_ms"], linewidth=4, label="OLS 95% CI")
    ax.plot([out["tau_ols_ms"]],  [y],       marker="o", ms=7, label=f"OLS {out['tau_ols_ms']:.1f} ms")
    ax.plot([out["tau_wls_ms"]],  [y + 0.08], marker="^", ms=7, label=f"WLS {out['tau_wls_ms']:.1f} ms")
    ax.plot([out["tau_tobit_ms"]],[y - 0.08], marker="s", ms=7, label=f"Tobit {out['tau_tobit_ms']:.1f} ms")
    ax.set_xlabel("τ (ms)")
    ax.set_yticks([])
...
def main():
    ...
    if not os.path.isfile(args.data):
        raise FileNotFoundError(f"Expected dataset not found: {args.data}\n"
                                f"Run decay/simulate_decay.py first.")
    ...
    _plot_tau_panel(out, args.out_png)
    print(f"Wrote {args.out_png}")

    # ------- Optional Monte-Carlo coverage over NEW datasets (fresh sims)
    R = int(args.repeats)
    if R > 0:
        from decay.wls_tobit_robustness import _simulate_once  # reuse your existing helper if present
        # If _simulate_once isn't available in this module, fall back to a quick inline version
        try:
            _simulate = _simulate_once  # type: ignore
        except Exception:
            # Inline sim (matches simulate_decay settings)
            def _simulate(seed, A0, tau_f, noise_log, delta_start, delta_end, delta_step, n_rep):
                rng = np.random.default_rng(seed)
        ...
        for r in range(R):
            ...
            out_r = _fit_all_from_dataset(df_r, A_min=A_min, n_boot=n_boot,
                                          ci_percent=ci_percent, seed=base_seed + 202 + r)
            rows.append(out_r)
