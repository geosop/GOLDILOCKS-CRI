...
# -*- coding: utf-8 -*-
"""
decay/wls_tobit_robustness.py  •  CRI v0.3-SIM (robust)
...
Robustness upgrades / hostile-reviewer hardening:
  ...
  2) Require decay/output/run_manifest.json and enforce:
       - YAML(decay) effective params hash == manifest.run_hash
       - decay_data.csv exists and has required columns
  3) Stamp run_hash + params_path into the output CSV and into figure metadata.
  4) Fix env-seed override bug from older versions.
  5) Optional Monte-Carlo repeats re-simulate NEW datasets; default stays strictly on the existing dataset.
...
def _effective_sim_params_from_yaml(p_raw: Dict[str, Any], resolved_path: str) -> Dict[str, Any]:
    """
...
def _plot_tau_panel(out: dict, out_png: str, out_pdf: str, run_hash: str, params_path: str):
    """
    ...
    # Provenance note in figure metadata (not visible unless inspected)
    ...
    fig.savefig(out_pdf, bbox_inches="tight", metadata=meta)
    plt.close(fig)
...
def main():
    ...
    sim_effective = _effective_sim_params_from_yaml(p_raw, params_path)
    ...
    _plot_tau_panel(out, args.out_png, args.out_pdf, run_hash=run_hash, params_path=params_path)
